...
namespace Olaf7
{
    internal class Program
    {
        ...
        static void OnLoadingComplete()
        {
            ...
            Titanic = new Item(ItemId.Titanic_Hydra, _Player.GetRealAutoAttackRange());
        ...
        public static void Combo()
        {
            ...
            if (target != null)
            {
                var pos = Q.GetPrediction(target).CastPosition.Extend(_Player.Position, -80);
        ...
        public static void ResetAttack(object e, OrbwalkerActionArgs args)
        {
            ...
            if (target != null)
            {
                ...
                if ((useE && E.IsReady()) && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo) && target.IsValidTarget(325) && _Player.Distance(target) < _Player.GetRealAutoAttackRange(target))
                {
                    E.Cast(target);
                }
            }
        ...
        public static void JungleClear()
        {
            ...
            if (jungle != null)
            {
                if (useQ && Q.IsReady() && Q.IsInRange(jungle) && _Player.ManaPercent >= mana)
                {
                    Q.Cast(jungle.Position);
                }

                if (useW && W.IsReady() && jungle.IsValidTarget(325) && _Player.ManaPercent >= mana)
                {
                    W.Cast();
                }
        ...
        private static void LaneClear()
        {
            ...
            if (quang.Position.IsValid())
                ...
                {
                    if (useW && W.IsReady() && _Player.ManaPercent >= mana && minion.IsValidTarget(E.Range))
                    {
                        W.Cast();
                    }

                    if (useE && E.IsReady() && !minion.IsValidTarget(_Player.AttackRange) && minion.Health <= _Player.GetSpellDamage(minion, SpellSlot.E))
                    {
                        E.Cast(minion);
                    }

                    if (useQ && Q.IsReady() && _Player.ManaPercent >= mana && minion.IsValidTarget(600))
                    {
        ...
        private static void Orbwalker_CantLasthit(object target, OrbwalkerActionArgs args)
        {
            ...
            var unit = (useCant && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LaneClear) && _Player.ManaPercent >= laneQMN)
            || (useAA && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LastHit) && _Player.ManaPercent >= LhM);
            ...
            {
                /*if (_Player.GetSpellDamage(target, SpellSlot.Q) >= HealthPrediction.Health.GetPrediction(target, Q.CastDelay))
                {
                    Q.Cast(target);
                }*/
            }
        ...
        public static void Harass()
        {
            ...
            {
                var pos = Q.GetPrediction(target).CastPosition.Extend(_Player.Position, -80);
                if (useQ && Q.IsReady() && target.IsValidTarget(Q.Range) && _Player.ManaPercent >= ManaQ)
                {
                    if (_Player.Distance(target) > 375)
                    {
                        Q.Cast(pos);
                ...
                if (useW && W.IsReady() && target.IsValidTarget(325) && _Player.ManaPercent >= ManaQ)
                {
                    W.Cast();
                }
        ...
        public static void LastHit()
        {
            ...
            var LhM = LastHitMenu["LhMana"].GetValue<MenuSlider>().Value;
            var minion = GameObjects.EnemyMinions.Where(a => a.Distance(_Player) <= Q.Range).OrderBy(a => a.Health).FirstOrDefault();
            if (minion != null)
            {
                if (useQ && Q.IsReady() && _Player.ManaPercent > LhM && minion.IsValidTarget(Q.Range) && _Player.GetSpellDamage(minion, SpellSlot.Q) >= minion.Health + minion.AllShield)
                {
                    Q.Cast(minion);
                }

                if (useE && E.IsReady() && _Player.GetSpellDamage(minion, SpellSlot.E) >= minion.Health + minion.AllShield)
                {
                    E.Cast(minion);
                }
            }
        ...
        public static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(Q.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead))
            {
                if (KsQ && Q.IsReady())
                {
                    var pos = Q.GetPrediction(target).CastPosition.Extend(_Player.Position, -80);
                    if (target.Health <= _Player.GetSpellDamage(target, SpellSlot.Q))
                    {
        ...
        private static void Ult()
        {
            ...
            var cc = (Misc["silence"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Silence))
            || (Misc["snare"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Snare))
            || (Misc["supperss"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Suppression))
            || (Misc["sleep"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Sleep))
            || (Misc["poly"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Polymorph))
            || (Misc["frenzy"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Frenzy))
            || (Misc["disarm"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Disarm))
            || (Misc["nearsight"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.NearSight))
            || (Misc["knockback"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Knockback))
            || (Misc["knockup"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Knockup))
            || (Misc["slow"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Slow))
            || (Misc["poison"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Poison))
            || (Misc["blind"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Blind))
            || (Misc["charm"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Charm))
            || (Misc["tunt"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Taunt))
            || (Misc["stun"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Stun))
            || (Misc["rot"].GetValue<MenuBool>().Enabled)
            || (Misc["fear"].GetValue<MenuBool>().Enabled && _Player.HasBuffOfType(BuffType.Fear));
            if (R.IsReady() && ulti && cc && _Player.Position.CountEnemyHeroesInRange(800) >= minR && _Player.HealthPercent <= heal)
            {
                R.Cast();
            }
        }
