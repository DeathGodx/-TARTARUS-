...
namespace Kassadin7
{
    internal class Program
    {
        ...
        {
            get { return _Player.HasBuff("ForcePulseE"); }
        }
        ...
        private static void Combo()
        {
            ...
            if (target != null)
            {
                ...
                {
                    if (_Player.Distance(target) < _Player.GetRealAutoAttackRange(target) && !W.IsReady())
                    {
                        Q.Cast(target);
                    }
                    else if (_Player.Distance(target) > _Player.GetRealAutoAttackRange(target))
                    {
                        Q.Cast(target);
                    }
                }
        ...
        private static void LaneClear()
        {
            ...
            if (qFarmLocation.Position.IsValid())
                foreach (var minions in minionQ)
                {
                    ...
                    && _Player.Distance(minions.Position) <= 225f
                    && _Player.GetSpellDamage(minions, SpellSlot.W) + _Player.GetAutoAttackDamage(minions)
                    >= minions.Health + minions.AllShield)
                    {
                        W.Cast();
                        _Player.IssueOrder(GameObjectOrder.AttackUnit, minions);
                    }


                    if (_Player.ManaPercent < mana) return;
                    if (useQ && Q.IsReady() && minions.IsValidTarget(Q.Range) && _Player.GetSpellDamage(minions, SpellSlot.Q) > minions.Health + minions.AllShield && (_Player.Distance(minions) > 225 || !W.IsReady()))
                    {
                        Q.Cast(minions);
                    }
                    ...
                    if (useR && R.IsReady() && minions.IsValidTarget(R.Range) && !UnderTuret(minions) && _Player.GetBuffCount("RiftWalk") < minRs)
                    {
                        R.Cast(minions);
                    }
                }
        ...
        private static void LastHit()
        {
            ...
            if (minion != null)
            {
                if (useQ && Q.IsReady() && minion.IsValidTarget(Q.Range) && _Player.GetSpellDamage(minion, SpellSlot.Q) > minion.Health + minion.AllShield
                && _Player.ManaPercent >= mana && (_Player.Distance(minion) > 225 || !W.IsReady()))
                {
                    Q.Cast(minion);
                }

                if (useW && W.IsReady() && minion.IsValidTarget(275) && minion.InAutoAttackRange()
                && _Player.Distance(minion.Position) <= 225f
                && _Player.GetSpellDamage(minion, SpellSlot.W) + _Player.GetAutoAttackDamage(minion)
        ...
        private static void Harass()
        {
            ...
            if (target != null)
            {
                ...
                if (useR && R.IsReady() && target.IsValidTarget(R.Range) && target.Position.CountEnemyHeroesInRange(R.Range) <= minR && _Player.HealthPercent >= Minhp && _Player.GetBuffCount("RiftWalk") < minRs)
                {
                    if (turret)
                    {
                        if (!UnderTuret(target))
        ...
        public static void JungleClear()
        {
            ...
            if (jungleMonsters != null)
            {
                ...
                if (useW && W.IsReady() && jungleMonsters.IsValidTarget(275) && jungleMonsters.InAutoAttackRange() && _Player.Distance(jungleMonsters.Position) <= 225f)
                {
                    W.Cast();
                }
                ...
                if (useR && R.IsReady() && jungleMonsters.IsValidTarget(R.Range) && _Player.GetBuffCount("RiftWalk") < minRs)
                {
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(Q.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead ))// && !hero.IsZombie))
            {
                ...
                {
                    if (target.Health <= _Player.GetSpellDamage(target, SpellSlot.E) && EReady)
                    {
                        E.Cast(target);
                    }
                }
                ...
                {
                    if (target.Health <= _Player.GetSpellDamage(target, SpellSlot.R))
                    {
                        R.Cast(target);
                    }
                }
