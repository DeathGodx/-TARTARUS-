...
namespace Sejuani7
{
    internal class Program
    {
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            {
                _Player.IssueOrder(GameObjectOrder.MoveTo, Game.CursorPos); //hata verirse Cursoru sil
            }
        }
        ...
        private static void Combo()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(R.Range) && !e.IsDead))
            {
                ...
                if (useE && E.IsReady() && target.IsValidTarget(E.Range) && disE <= target.Distance(_Player))
                {
                    E.CastOnUnit(target);
                }
        ...
        private static void FlashQ()
        {
            ...
            if (targetF != null)
            {
                ...
                if (_Player.HasBuff("SejuaniArcticAssault") && targetF.IsValidTarget(425))
                {
                    //_Player.CastSpell(_Player.GetSpellSlot("summonerflash"), targetF.Position);
                }
            }
        }
        ...
        private static void LaneClear()
        {
            ...
            var minion = GameObjects.EnemyMinions.Where(a => a.Distance(_Player) <= E.Range).OrderBy(a => a.Health).FirstOrDefault();
