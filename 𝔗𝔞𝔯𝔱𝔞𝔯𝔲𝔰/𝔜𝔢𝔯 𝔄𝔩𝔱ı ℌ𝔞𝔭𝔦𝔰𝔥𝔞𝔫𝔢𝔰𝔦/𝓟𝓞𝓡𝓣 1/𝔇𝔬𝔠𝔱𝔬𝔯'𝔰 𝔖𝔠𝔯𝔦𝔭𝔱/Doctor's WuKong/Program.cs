...
namespace Doctor_s_WuKong
{
    static class Program
    {
        ...
        {
            get { return _Player.HasBuff("monkeykingdoubleattack"); }
        }

        public static bool RActive
        {
            get { return _Player.HasBuff("MonkeyKingSpinToWin"); }
        }
        ...
        private static void Combo()
        {
            ...
            if (target != null)
            {
                if (useE && E.IsReady() && !RActive && target.IsValidTarget(E.Range) && disE <= target.Distance(_Player))
                {
                    if (turret)
                    {
        ...
        private static void Ultimate()
        {
            ...
            if (target != null)
            {
                ...
                if (useR2 && R.IsReady() && !RActive && _Player.HealthPercent <= mauR && _Player.Position.CountEnemyHeroesInRange(R.Range) >= 1 && !_Player.InShop())
                {
                    R.Cast();
                }

                if (autow && W.IsReady() && !RActive && _Player.HealthPercent <= mauW && _Player.Position.CountEnemyHeroesInRange(R.Range) >= 1 && !_Player.InShop())
                {
                    W.Cast();
                }
        ...
        private static void LaneClear()
        {
            ...
            foreach (var minion in minions)
            {
                ...
                && _Player.Distance(minion.Position) <= 300
                && _Player.GetSpellDamage(minion, SpellSlot.Q) + _Player.GetAutoAttackDamage(minion)
        ...
        private static void Harass()
        {
            ...
            var target = TargetSelector.GetTarget(E.Range, DamageType.Physical);
            if (target != null && _Player.ManaPercent >= mana)
            {
        ...
        public static void Flee()
        {
            if (E.IsReady())
            {
                var CursorPos = Game.CursorPos.ToVector2();
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(E.Range) && !hero.HasBuff("BlitzcrankManaBarrierCD") && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead ))//&& !hero.IsZombie))
            {
                ...
                {
                    if (target.Health + target.Health + target.AllShield < _Player.GetSpellDamage(target, SpellSlot.E) + _Player.GetSpellDamage(target, SpellSlot.Q))
                    {
                        E.Cast(target);
                    }
                }
                ...
                {
                    if (target.Health + target.Health + target.AllShield < _Player.GetSpellDamage(target, SpellSlot.Q))
                    {
                        Q.Cast();
                    }
                }
                ...
                {
                    if (target.Health + target.Health + target.AllShield < _Player.GetSpellDamage(target, SpellSlot.R))
                    {
                        R.Cast();
                    }
                }
