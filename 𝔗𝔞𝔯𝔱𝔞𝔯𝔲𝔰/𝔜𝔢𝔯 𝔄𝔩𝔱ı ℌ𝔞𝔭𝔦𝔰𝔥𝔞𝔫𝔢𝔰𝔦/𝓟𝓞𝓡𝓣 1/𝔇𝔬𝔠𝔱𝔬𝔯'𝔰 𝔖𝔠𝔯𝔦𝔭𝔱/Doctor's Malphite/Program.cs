    ...
    internal class Program
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Drawings["Notifications"].GetValue<MenuBool>().Enabled && R.IsReady())
            {
                ...
                if (target != null && target.IsValidTarget(R.Range) && _Player.GetSpellDamage(target, SpellSlot.R) > target.Health + target.PhysicalShield)
                {
        ...
        private static void Combo()
        {
            ...
            {
                if (useQ && Q.IsReady() && target.IsValidTarget(Q.Range) && disQ < target.Distance(_Player))
                {
                    Q.Cast(target);
                }
        ...
        private static void LastHit()
        {
            ...
            if (minion != null)
            {
                if (useQ && Q.IsReady() && minion.IsValidTarget(Q.Range) && minion.Health <= _Player.GetSpellDamage(minion, SpellSlot.Q) && _Player.Distance(minion) > 175)
                {
                    Q.Cast(minion);
                }
            }
        ...
        private static void Harass()
        {
            ...
            if (_Player.ManaPercent < ManaQ) return;
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(R.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead ))//&& !hero.IsZombie))
            {
                ...
                {
                    if (target.Health + target.PhysicalShield <= _Player.GetSpellDamage(target, SpellSlot.E))
                    {
                        E.Cast();
                    }
                }
                ...
                if (KsR && R.IsReady())
                {
                    if (target.Health + target.PhysicalShield <= _Player.GetSpellDamage(target, SpellSlot.R) && !target.IsValidTarget(minKsR))
                    {
                        R.Cast(target);
                    }
                }
