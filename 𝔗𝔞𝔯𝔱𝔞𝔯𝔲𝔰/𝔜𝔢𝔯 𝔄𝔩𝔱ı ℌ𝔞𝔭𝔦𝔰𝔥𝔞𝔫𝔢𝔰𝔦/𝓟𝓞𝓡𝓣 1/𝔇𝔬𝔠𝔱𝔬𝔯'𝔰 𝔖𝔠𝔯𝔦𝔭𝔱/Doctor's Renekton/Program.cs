...
namespace Renekton7
{
    internal class Program
    {
        ...
        {
            get { return _Player.HasBuff("renektonpreexecute"); }
        }

        public static bool PassiveE
        {
            get { return _Player.HasBuff("RenekthonSliceAndDiceDelay"); }
        }

        public static bool Fury
        {
            get { return _Player.HasBuff("renektonrageready"); }
        }
        ...
        private static void Combo()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(E.Range) && !e.IsDead))
            {
                ...
                if (!PassiveE && useE && E.IsReady() && target.IsValidTarget(E.Range) && (200 <= target.Distance(_Player) || !Q.IsReady() && !W.IsReady()))
                {
                    E.Cast(target.Position);
                }

                if (useE2 && E.IsReady() && target.IsValidTarget(E.Range) && PassiveE && E2dis <= target.Distance(_Player))
                {
                    E.Cast(target.Position);
                }
            }
        ...
        private static void Ultimate()
        {
            ...
            if (useR && _Player.HealthPercent <= minR && _Player.Position.CountEnemyHeroesInRange(500) >= 1 && !_Player.InShop())
            {
                R.Cast();
            }

            if (useR2 && !_Player.InShop() && _Player.Position.CountEnemyHeroesInRange(450) >= minE)
            {
                R.Cast();
            }
        }
        ...
        private static void LaneClear()
        {
            ...
            foreach (var minion in minions)
            {
                ...
                && WDamage(minion) * 2 + _Player.GetAutoAttackDamage(minion)
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(E.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead ))
            {
