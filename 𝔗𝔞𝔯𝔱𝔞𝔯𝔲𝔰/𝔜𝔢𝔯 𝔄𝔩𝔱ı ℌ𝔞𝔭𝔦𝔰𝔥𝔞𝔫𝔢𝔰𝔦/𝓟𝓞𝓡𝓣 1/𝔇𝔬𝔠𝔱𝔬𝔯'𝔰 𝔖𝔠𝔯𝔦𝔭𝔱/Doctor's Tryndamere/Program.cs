...
namespace Tryndamere
{
    ...
    {
        private static AIHeroClient User = _Player;
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                Vector2 ft = Drawing.WorldToScreen(_Player.Position);
                if (_Player.HasBuff("Undying Rage"))
                {
                    DrawFont(thn, "Undying Rage : " + RTime(_Player), (float)(ft[0] - 125), (float)(ft[1] + 100), SharpDX.Color.Orange);
                }
            }
        }
        ...
        private static void Damage(EventArgs args)
        {
            ...
            {
                var damage = _Player.GetAutoAttackDamage(enemy) * 2;
        ...
        private static void Combo()
        {
            ...
            if (target != null)
            {
                if (useE && E.IsReady() && target.IsValidTarget(E.Range) && (disE <= target.Distance(_Player) || _Player.HealthPercent <= 20))
                {
                    if (turret)
                    {
                ...
                {
                    if (_Player.Distance(target) <= target.GetRealAutoAttackRange() && _Player.HealthPercent <= 60)
                    {
                        W.Cast();
                    }
                    ...
                    if (useW2)
                    {
                        if (!target.IsFacing(_Player) && _Player.Distance(target) >= 325 && !E.IsReady())
                        {
                            W.Cast();
                        }
                    }
                    ...
                    {
                        if (!target.IsFacing(_Player) && _Player.Distance(target) >= 325)
                        {
                            W.Cast();
                        }
                    }
        ...
        private static void Ultimate()
        {
            ...
            {
                if (!_Player.HasBuff("JudicatorIntervention") && !_Player.HasBuff("kindredrnodeathbuff"))
                {
                    if (useR && R.IsReady() && !_Player.InShop() && (target.IsValidTarget(E.Range) || _Player.IsUnderEnemyTurret()))
                    {
                        ...
                        /*if (_Player.HasBuff("ZedR"))
                        {
                            Core.DelayAction(() => R.Cast(), 500);
                        }*/
                    }
                }
                if (useQ && Q.IsReady() && RTime(_Player) <= 1 && _Player.Mana >= passive)
                {
                    if (useQ2)
                    {
                        if (!R.IsReady() && (_Player.HealthPercent <= mauQ || _Player.HasBuff("ZedR")))
                        {
                            Q.Cast();
                        }
                    }
                    ...
                    {
                        if ((_Player.HealthPercent <= mauQ || _Player.HasBuff("ZedR")))
                        {
                            Q.Cast();
                        }
                    }
