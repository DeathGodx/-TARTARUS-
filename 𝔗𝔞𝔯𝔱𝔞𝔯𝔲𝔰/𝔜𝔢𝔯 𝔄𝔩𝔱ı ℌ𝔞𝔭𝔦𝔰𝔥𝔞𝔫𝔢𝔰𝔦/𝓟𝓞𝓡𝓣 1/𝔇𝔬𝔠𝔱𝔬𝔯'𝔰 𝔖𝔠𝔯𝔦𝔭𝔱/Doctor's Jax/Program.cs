...
namespace Jax
{
    internal class Program
    {
        ...
        {
            get { return _Player.HasBuff("JaxCounterStrike"); }
        }
        ...
        public static void Item()
        {
            ...
            foreach (var targeti in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(475) && !e.IsDead))
            {
                ...
                if ((item && Botrk.IsReady && Botrk.IsOwned() && targeti.IsValidTarget(475)) && (_Player.HealthPercent <= Minhp || targeti.HealthPercent < Minhpp))
                {
                    Botrk.Cast(targeti);
                }
            }

            if (useR && R.IsReady() && _Player.Position.CountEnemyHeroesInRange(Q.Range) >= 1 && _Player.HealthPercent <= MinR)
            {
                R.Cast();
            }
        ...
        public static void Combo()
        {
            ...
            if (ComboMenu["comboMode"].GetValue<MenuList>().Index == 0)
            {
                if (target != null)
                {
                    ...
                    if (useQ && Q.IsReady() && target.IsValidTarget(Q.Range) && (_Player.GetRealAutoAttackRange() < target.Distance(_Player) || _Player.HealthPercent <= 25))
                    {
                        if (ECasting)
                        {
                            Q.Cast(target);
                ...
                if (target != null)
                {
                    if (useW && W.IsReady() && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                    {
                        ...
                        _Player.IssueOrder(GameObjectOrder.AttackUnit, targetz);
                    }
            ...
            if (ComboMenu["comboMode"].GetValue<MenuList>().Index == 1)
            {
                if (target != null)
                {
                    if (useQ && Q.IsReady() && _Player.GetRealAutoAttackRange() < target.Distance(_Player))
                    {
        ...
        public static void JungleClear()
        {
            ...
            if (jungleMonsters != null && _Player.ManaPercent >= mana)
            {
                ...
                {
                    W.Cast();
                    _Player.IssueOrder(GameObjectOrder.AttackUnit, jungleMonsters);
                }
        ...
        public static void LaneClear()
        {
            ...
            {
                if (useQ && Q.IsReady() && Q.IsInRange(minion) && _Player.GetRealAutoAttackRange() <= minion.Distance(_Player) && _Player.GetSpellDamage(minion, SpellSlot.Q) >= minion.Health + minion.AllShield)
                {
                    Q.Cast(minion);
                }
        ...
        {
            return WardIds.Select(wardId => _Player.InventoryItems.FirstOrDefault(a => a.Id == wardId)).FirstOrDefault(slot => slot != null );//&& slot.CanUseItem());
        }
        ...
        public static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(Q.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.IsDead ))//&& !hero.IsZombie))
            {
                ...
                {
                    if (target.Health <= _Player.GetSummonerSpellDamage(target, SummonerSpell.Ignite) && (_Player.GetAutoAttackDamage(target) < target.Health || !target.IsValidTarget(_Player.GetRealAutoAttackRange())))
                    {
                        Ignite.Cast(target);
                    }
                }
