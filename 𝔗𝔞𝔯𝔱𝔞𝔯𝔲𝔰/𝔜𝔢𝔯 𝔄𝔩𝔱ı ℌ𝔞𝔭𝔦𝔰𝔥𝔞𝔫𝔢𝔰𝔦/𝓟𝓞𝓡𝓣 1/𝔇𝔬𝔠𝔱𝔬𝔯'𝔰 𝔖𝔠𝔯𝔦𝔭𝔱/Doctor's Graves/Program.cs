    ...
    internal static class Program
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Drawings["Notifications"].GetValue<MenuBool>().Enabled && R.IsReady())
            {
                ...
                if (target.IsValidTarget(R.Range) && _Player.GetSpellDamage(target, SpellSlot.R) > target.Health + target.PhysicalShield)
                {
        ...
        private static void ResetAttack(object target, OrbwalkerActionArgs args)
        {
            ...
            if (useJ && E.IsReady() && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LaneClear) && _Player.ManaPercent >= manaJ)
            {
                E.Cast(Game.CursorPos);
                Orbwalker.ResetAutoAttackTimer();
            }

            if (useL && E.IsReady() && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LaneClear) && _Player.ManaPercent >= mana)
            {
                E.Cast(Game.CursorPos);
                Orbwalker.ResetAutoAttackTimer();
            }

            if (HarassMenu["HarassAA"].GetValue<MenuBool>().Enabled && E.IsReady() && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Harass) && _Player.ManaPercent >= HarassMenu["ManaHarass"].GetValue<MenuSlider>().Value)
            {
                E.Cast(Game.CursorPos);
                Orbwalker.ResetAutoAttackTimer();
            }
        ...
        public static void Combo()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(R.Range) && !e.IsDead)) //&& !e.IsZombie))
            {
                ...
                if (useE && E.IsReady())
                {
                    if (ComboMode.Value == 1 && !_Player.HasBuff("GravesBasicAttackAmmo2") && _Player.Position.CountEnemyHeroesInRange(R.Range) >= 1)
                    {
                        E.Cast(Game.CursorPos);
                    }
                    //Olmazsa CursorPosCenteri düzelt Sil)
                    if (ComboMode.Value == 0 && !_Player.HasBuff("GravesBasicAttackAmmo2") && !_Player.HasBuff("GravesBasicAttackAmmo1") && _Player.Position.CountEnemyHeroesInRange(R.Range) >= 1)
                    {
                        E.Cast(Game.CursorPos);
                    }
                }
        ...
        private static void Harass()
        {
            ...
            if (_Player.ManaPercent < ManaW) return;
        ...
        private static void LaneClear()
        {
            ...
            {
                if (useQ && Q.IsReady() && minion.IsValidTarget(Q.Range) && _Player.ManaPercent > ManaQ && quang.MinionsHit >= MinQ)
                {
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(R.Range) && !e.HasBuff("BlitzcrankManaBarrierCD") && !e.HasBuff("JudicatorIntervention") && !e.HasBuff("kindredrnodeathbuff") && !e.HasBuff("Undying Rage") && !e.IsDead))// && !e.IsZombie))
            {
                ...
                if (KsR && R.IsReady())
                {
                    if (target.Health + target.PhysicalShield < _Player.GetSpellDamage(target, SpellSlot.R) && target.IsValidTarget(R.Range) && !target.IsValidTarget(minKsR))
                    {
                        var pred = R.GetPrediction(target);
                ...
                {
                    if (target.Health + target.PhysicalShield < _Player.GetSpellDamage(target, SpellSlot.R))
                    {
                        var pred = R.GetPrediction(target);
                        if (pred.Hitchance >= HitChance.VeryHigh)
                        {
