    ...
    static class Program
    {
        ...
        private static void ResetAttack(object e, OrbwalkerActionArgs args)
        {
            ...
            if (target != null)
            {
                ...
                {
                    if (useQ && Q.IsReady() && target.IsValidTarget(Q.Range) && _Player.Distance(target) < _Player.GetRealAutoAttackRange(target) && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                    {
                ...
                {
                    if (useR && R.IsReady() && R.Ammo >= 1 && target.IsValidTarget(R.Range) && _Player.Distance(target) < _Player.GetRealAutoAttackRange(target) && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                    {
        ...
        private static void KillSteal()
        {
            ...
            {
                if (useR && R.IsReady() && target.Health <= _Player.GetSpellDamage(target, SpellSlot.R))
                {
                    var RPred = R.GetPrediction(target);
                    if (RPred.Hitchance >= HitChance.Medium)
                    {
                ...
                if (useQ && Q.IsReady() && target.Health <= _Player.GetSpellDamage(target, SpellSlot.Q))
                {
                    var Pred = Q.GetPrediction(target);
                    if (Pred.Hitchance >= HitChance.Medium)
                    {
