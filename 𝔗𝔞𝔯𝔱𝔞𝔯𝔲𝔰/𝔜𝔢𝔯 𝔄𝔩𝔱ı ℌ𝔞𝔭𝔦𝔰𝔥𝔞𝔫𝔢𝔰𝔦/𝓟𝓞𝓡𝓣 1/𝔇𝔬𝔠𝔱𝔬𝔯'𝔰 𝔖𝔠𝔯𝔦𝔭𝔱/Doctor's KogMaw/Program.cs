...
namespace KogMaw
{
    internal class Program
    {
        ...
        static void OnLoadingComplete()
        {
            ...
            W = new Spell(SpellSlot.W, (uint)_Player.GetRealAutoAttackRange());
            E = new Spell(SpellSlot.E, 1200);
            R = new Spell(SpellSlot.R, 900 + 300 * (uint)_Player.Spellbook.GetSpell(SpellSlot.R).Level);
        ...
        private static void Combo()
        {
            ...
            if (target != null)
            {
                ...
                if (useW && W.IsReady() && target.IsValidTarget(610 + 20 * (uint)_Player.Spellbook.GetSpell(SpellSlot.W).Level))
                {
                    W.Cast();
                }
            }
        ...
        private static void Ultimate()
        {
            ...
            if (useR && R.IsReady() && _Player.GetBuffCount("kogmawlivingartillerycost") < Rlimit && _Player.ManaPercent > mana)
            {
        ...
        private static void LaneClear()
        {
            ...
            {
                if (useQ && Q.IsReady() && minions.IsValidTarget(Q.Range) && _Player.GetSpellDamage(minions, SpellSlot.Q) > minions.Health + minions.AllShield && _Player.Distance(minions) > 500)
                {
                    Q.Cast(minions);
                }

                if (useR && R.IsReady() && minions.IsValidTarget(R.Range) && _Player.GetBuffCount("kogmawlivingartillerycost") <= Rlimit)
                {
                    R.Cast(minions);
                }
        ...
        private static void Harass()
        {
            ...
            if (target != null)
            {
                ...
                if (R.IsReady() && useR && _Player.GetBuffCount("kogmawlivingartillerycost") < Rlimit)
                {
                    var Rpred = R.GetPrediction(target);
        ...
        public static void JungleClear()
        {
            ...
            if (monster != null)
            {
                ...
                if (useR && R.IsReady() && _Player.GetBuffCount("kogmawlivingartillerycost") < Rlimit)
                {
                    R.Cast(monster);
                }
            }
        ...
        public static void Flee()
        {
            ...
            if (target != null)
            {
                if (E.IsReady() && target.IsValidTarget(E.Range) && !target.IsDead)
                {
                    E.Cast(target);
                }
            }
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(R.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead))
            {
