    ...
    internal class Program
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Drawings["DrawT"].GetValue<MenuBool>().Enabled)
            {
                ...
                if (_Player.HasBuff("HecarimRamp"))
                {
                    DrawFont(thn, "E Time : " + ETime(_Player), (float)(ft[0] - 70), (float)(ft[1] + 50), SharpDX.Color.GreenYellow);
                }
            }
        ...
        private static void LaneClear()
        {
            ...
            var minion = GameObjects.EnemyMinions.Where(a => a.Distance(_Player) <= W.Range).OrderBy(a => a.Health).FirstOrDefault();
            if (minion != null)
            {
                if (useQLH && Q.IsReady() && minion.IsValidTarget(Q.Range) && _Player.ManaPercent >= laneQMN)
                {
                    Q.Cast();
                }

                if (useQ && Q.IsReady() && minion.IsValidTarget(Q.Range) && minion.Health < QDamage(minion) && _Player.ManaPercent >= laneQMN)
                {
                    Q.Cast();
                }

                if (useWLH && W.IsReady() && minion.IsValidTarget(W.Range) && _Player.ManaPercent >= laneWMN)
                {
                    W.Cast();
                }
            }
        ...
        private static void Harass()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(W.Range) && !e.IsDead))
            {
                ...
                if (useW && W.IsReady() && target.IsValidTarget(W.Range) && _Player.ManaPercent >= ManaW)
                {
                    W.Cast();
                }
            }
        ...
        private static void AutoQ()
        {
            ...
            {
                if (useQ && Q.IsReady() && !Tru(_Player.Position) && _Player.ManaPercent >= mana && target.IsValidTarget(Q.Range) && !Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo) && !Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Harass))
                {
                    Q.Cast();
                }
            }
        ...
        private static void LastHit()
        {
            ...
            if (minion != null)
            {
                if (useQ && Q.IsReady() && _Player.ManaPercent >= mana && minion.IsValidTarget(Q.Range) && minion.Health < QDamage(minion))
                {
                    Q.Cast();
                }
            }
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(R.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead)) //&& !hero.IsZombie))
            {
                ...
                {
                    if (target.Health + target.AllShield <= _Player.GetSpellDamage(target, SpellSlot.Q))
                    {
                        Q.Cast();
                    }
                }
                ...
                {
                    if (target.Health + target.AllShield <= _Player.GetSpellDamage(target, SpellSlot.W))
                    {
                        W.Cast();
                    }
                }
                ...
                {
                    if (target.Health + target.AllShield <= RDamage(target) && (!target.IsValidTarget(minKsR) || _Player.HealthPercent <= 25))
                    {
                        var RPred = R.GetPrediction(target);
                        if (RPred.Hitchance >= HitChance.High)
                        {
