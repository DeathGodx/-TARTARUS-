    ...
    internal class Program
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Misc["DrawT"].GetValue<MenuBool>().Enabled)
            {
                ...
                if (_Player.HasBuff("TwitchHideInShadows"))
                {
                    DrawFont(thm, "Q Stealthed : " + QTime(_Player), (float)(ft[0] - 100), (float)(ft[1] + 50), SharpDX.Color.GreenYellow);
                }
            }
            ...
            if (Misc["DrawT"].GetValue<MenuBool>().Enabled)
            {
                ...
                {
                    DrawFont(thm, "R Time : " + RTime(_Player), (float)(ft[0] - 70), (float)(ft[1] + 100), SharpDX.Color.Red);
                }
            }
        }
        ...
        {
            get { return _Player.HasBuff("TwitchHideInShadows"); }
        }
        ...
        public static void Combo()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(E.Range) && !e.IsDead ))//&& !e.IsZombie))
            {
                ...
                if (useW && W.IsReady() && !QCasting && target.IsValidTarget(W.Range) && _Player.Distance(target) > _Player.GetRealAutoAttackRange(target))
                {
        ...
        public static void ResetAttack(object e, OrbwalkerActionArgs args)
        {
            ...
            if (target != null)
            {
                if (useW && W.IsReady() && target.IsValidTarget(W.Range) && _Player.Distance(target) < _Player.GetRealAutoAttackRange(target) && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                {
                    var Pred = W.GetPrediction(target);
        ...
        public static void Item()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(900) && !e.IsDead))
            {
                ...
                if (yous && Youmuu.IsReady && Youmuu.IsOwned() && _Player.Distance(target) <= _Player.GetRealAutoAttackRange() && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                {
                    if (_Player.HasBuff("TwitchFullAutomatic"))
                    {
                        Youmuu.Cast();
        ...
        private static void Orbwalker_CantLasthit(object targeti, OrbwalkerActionArgs args)
        {
            ...
            var unit = (useE && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LaneClear) && _Player.ManaPercent >= mana);
        ...
        public static void Escape()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(E.Range) && !hero.HasBuff("FioraW") && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.HasBuff("SpellShield") && !hero.HasBuff("NocturneShield") && hero.HasBuff("twitchdeadlyvenom") && !hero.IsDead))// && !hero.IsZombie))
            {
                ...
                if (E.IsReady() && _Player.HealthPercent <= 15)
                {
                    E.Cast();
                }
            }
