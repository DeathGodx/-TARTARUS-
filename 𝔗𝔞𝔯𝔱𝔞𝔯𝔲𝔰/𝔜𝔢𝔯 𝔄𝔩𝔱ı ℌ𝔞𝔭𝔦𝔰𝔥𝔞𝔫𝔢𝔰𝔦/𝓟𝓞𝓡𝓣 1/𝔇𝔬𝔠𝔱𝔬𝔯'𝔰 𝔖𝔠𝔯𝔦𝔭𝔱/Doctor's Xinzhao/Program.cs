...
namespace XinZhao7
{
    static class Program
    {
        ...
        static void OnLoadingComplete()
        {
            ...
            Game.OnUpdate+= Game_OnTick;//onprocces packet denersin çalışmazsa
        ...
        private static void Combo()
        {
            ...
            if (target != null)
            {
                if (useE && E.IsReady() && target.IsValidTarget(E.Range) && (disE <= target.Distance(_Player) || target.IsDashing()))
                {
                    if (turret)
                    {
                ...
                if ((item && Botrk.IsReady && Botrk.IsOwned() && target.IsValidTarget(450)) && (_Player.HealthPercent <= Minhp || target.HealthPercent < Minhpp))
                {
                    Botrk.Cast(target);
                }
            }
        ...
        private static void Ultimate()
        {
            ...
            if (useR && !_Player.InShop() && _Player.Position.CountEnemyHeroesInRange(R.Range) >= minR)
            {
                R.Cast();
            }

            if (useR2 && _Player.HealthPercent <= mauR && _Player.Position.CountEnemyHeroesInRange(R.Range) >= 1 && !_Player.InShop())
            {
                R.Cast();
            }
        }
        ...
        public static void Interupt(object sender, InterruptSpellArgs i)
        {
            ...
            if (!(sender is AIHeroClient) || _Player.IsRecalling())
            {
                return;
            }
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(R.Range) && !hero.HasBuff("FioraW") && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.HasBuff("SpellShield") && !hero.HasBuff("NocturneShield") && !hero.IsDead ))//&& !hero.IsZombie))
            {
                ...
                {
                    if (target.Health + target.Health + target.AllShield < _Player.GetSpellDamage(target, SpellSlot.E))
                    {
                        E.Cast(target);
                    }
                }
