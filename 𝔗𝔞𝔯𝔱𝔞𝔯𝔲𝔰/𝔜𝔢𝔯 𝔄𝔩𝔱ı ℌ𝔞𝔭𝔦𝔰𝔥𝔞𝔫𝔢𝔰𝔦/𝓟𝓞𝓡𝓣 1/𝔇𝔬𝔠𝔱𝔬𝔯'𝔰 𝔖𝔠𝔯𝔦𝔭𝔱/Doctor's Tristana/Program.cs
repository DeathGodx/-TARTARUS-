...
namespace Tristana
{
    static class Program
    {
        ...
        public static double EDamage(AIBaseClient target)
        {
            ...
            {
                Edamage += (float)(_Player.GetSpellDamage(target, SpellSlot.E) * (target.GetBuffCount("tristanaecharge") * 0.30)) + _Player.GetSpellDamage(target, SpellSlot.E);
            }
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target2 in target)
            {
                ...
                {
                    if (target2.Health + target2.AllShield <= _Player.GetAutoAttackDamage(target2) * WAttack && _Player.HealthPercent >= 10 && target2.Position.CountEnemyHeroesInRange(600) <= minW)
                    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Misc["Notifications"].GetValue<MenuBool>().Enabled && R.IsReady())
            {
                ...
                if (target.IsValidTarget(1000) && _Player.GetSpellDamage(target, SpellSlot.R) > target.Health + target.PhysicalShield)
                {
