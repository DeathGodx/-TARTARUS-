    ...
    class Program
    {
        ...
        private static void OnLoadingComplete()
        {
           ...
           // Chat.Print("Doctor's Yi Loaded! Ported By DEATHGODX", Color.Orange);
            ...
            foreach (var enemies in GameObjects.EnemyHeroes.Where(a => a.Team != _Player.Team))
            {
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            {
                if (_Player.HasBuff("Meditate") == false && W.IsReady() == false)
                {
        ...
        public static void Combo()
        {
            ...
            if (target != null)
            {
                if (useQ && Q.IsReady() && target.IsValidTarget(Q.Range) && (_Player.Distance(target) > 250 || target.IsDashing() || _Player.HealthPercent <= 20))
                {
                    Q.Cast(target);
                }
                ...
                if (useR && R.IsReady() && (_Player.Position.CountEnemyHeroesInRange(675) >= MinR || _Player.HealthPercent <= 30))
                {
                    R.Cast();
                }

                if (useW && _Player.HasBuff("Meditate"))
                {
                    if (_Player.Distance(target) <= _Player.GetRealAutoAttackRange(target))
                    {
        ...
        public static void LaneClear()
        {
            ...
            foreach (var minions in minion)
            {
                ...
                {
                    int ECal = minion.Where(e => e.Distance(_Player.Position) <= Q.Range && _Player.GetSpellDamage(e, SpellSlot.Q) >= e.Health).Count(); ;
                    if (ECal >= minE)
                    {
                        Q.Cast(minions);
                    }
        ...
        public static void ResetAttack(object e, OrbwalkerActionArgs args)
        {
            ...
            if (target != null)
            {
                if (useW && W.IsReady() && _Player.Distance(target) <= _Player.GetRealAutoAttackRange() - 50 && _Player.ManaPercent > mana && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Harass))
                {
                    W.Cast();
                    Orbwalker.CanAttack();
                    _Player.IssueOrder(GameObjectOrder.AttackUnit, target);
                ...
                if (useQC && W.IsReady() && _Player.Distance(target) <= _Player.GetRealAutoAttackRange() - 50 && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                {
                    W.Cast();
                    Orbwalker.CanAttack();
                    _Player.IssueOrder(GameObjectOrder.AttackUnit, target);
        ...
        private static void AIHeroClient_OnProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            ...
            {
                if (_Player.HealthPercent <= MinHealth)
                {
                    W.Cast();
                }
            }
                 ...
                 args.Slot == SpellSlot.R) && sender.IsEnemy && Q.IsReady() && _Player.Distance(sender) <= args.SData.CastRange && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
            {
                ...
                {
                    if ((args.Target.NetworkId == _Player.NetworkId && args.Time < 1.5 ||
                         args.End.Distance(_Player.Position) <= _Player.BoundingRadius * 3) &&
