...
namespace Kayle
{
    internal class Program
    {
        ...
        private static void OnLoadingComplete()
        {
            ...
            E = new Spell(SpellSlot.E, (uint)_Player.GetRealAutoAttackRange());
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Misc["DrawE"].GetValue<MenuBool>().Enabled && _Player.HasBuff("JudicatorRighteousFury") && E.IsReady())
            {
            ...
            if (Misc["DrawIE"].GetValue<MenuBool>().Enabled)
            {
                ...
                if (_Player.HasBuff("JudicatorRighteousFury"))
                {
                    DrawFont(thm, "Righteous Fury : " + ETime(_Player), (float)(ft[0] - 125), (float)(ft[1] + 50), SharpDX.Color.Pink);
                }
            }
        }
        ...
        private static void Ultimate()
        {
            ...
            var target = GameObjects.AllyHeroes.Where(a => a.IsValidTarget() && a.Distance(_Player.Position) <= W.Range && !a.IsDead && !a.HasBuff("kindredrnodeathbuff") && !a.HasBuff("JudicatorIntervention") && !a.HasBuff("Recall"));
            if (_Player.ManaPercent <= mana) return;
            foreach (var target2 in target)
            {
                if (useW2 && W.IsReady() && !_Player.InShop() && !_Player.IsRecalling())
                {
        ...
        private static void Heals()
        {
            ...
            var useW2 = Heal["healW2"].GetValue<MenuBool>().Enabled;
            var mana = Heal["ManaHeal"].GetValue<MenuSlider>().Value;
            var target = GameObjects.EnemyHeroes.Where(a => a.IsValidTarget() && a.Distance(_Player.Position) <= W.Range && !a.IsDead && !a.HasBuff("kindredrnodeathbuff") && !a.HasBuff("JudicatorIntervention") && !a.HasBuff("Recall"));
            if (_Player.ManaPercent <= mana) return;
            foreach (var target2 in target)
            {
                if (useW2 && W.IsReady() && !_Player.InShop() && !_Player.IsRecalling())
            ...
            if (_Player.HealthPercent <= almin && W.IsReady())
            {
                W.Cast();
            }
        }
