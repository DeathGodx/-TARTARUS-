    ...
    internal class Program
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Drawings["Notifications"].GetValue<MenuBool>().Enabled && R.IsReady())
            {
                ...
                if (target.IsValidTarget(1600) && _Player.GetSpellDamage(target, SpellSlot.R) > target.Health + target.PhysicalShield)
                {
        ...
        public static void ResetAttack(object e, OrbwalkerActionArgs args)
        {
            ...
            if (target != null)
            {
                ...
                {
                    if (useQ && Q.IsReady() && target.IsValidTarget(Q.Range) && _Player.Position.Distance(target) < _Player.GetRealAutoAttackRange(target) && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                    {
                ...
                {
                    if (useW && W.IsReady() && target.IsValidTarget(W.Range) && _Player.Position.Distance(target) < _Player.GetRealAutoAttackRange(target) && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                    {
        ...
        public static void Combo()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(2000) && !e.IsDead))
            {
                if (useQ && Q.IsReady() && target.IsValidTarget(Q.Range))
                {
                    ...
                    {
                        if (_Player.Position.Distance(target) > _Player.GetRealAutoAttackRange(target))
                        {
        ...
        public static void LaneClear()
        {
            ...
            if (minions != null)
            {
                ...
                {
                    if (useQ && _Player.ManaPercent >= mana)
                    {
                        if (Q.IsReady() && minions.Health <= _Player.GetSpellDamage(minions, SpellSlot.Q))
                        {
        ...
        public static void Harass()
        {
            ...
            {
                if (useQ && _Player.ManaPercent >= ManaQ && Q.IsReady() && Selector.IsValidTarget(Q.Range))
                {
                ...
                if (useW && _Player.ManaPercent >= ManaW && W.IsReady() && Selector.IsValidTarget(W.Range))
                {
        ...
        public static void LastHit()
        {
            ...
            if (minion != null)
            {
                ...
                {
                    if (useQ && Q.IsReady() && minion.Health + minion.AllShield <= _Player.GetSpellDamage(minion, SpellSlot.Q))
                    {
                        Q.Cast(minion);
                    }
                }
        ...
        public static void AutoHarass()
        {
            ...
            foreach (var Selector in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(W.Range) && !e.IsDead))
            {
                ...
                {
                    if (useQ && Q.IsReady() && automana <= _Player.ManaPercent)
                    {
                    ...
                    if (useW && W.IsReady() && automanaw <= _Player.ManaPercent)
                    {
        ...
        public static void KillSteal()
        {
...

            {
                ...
                {
                    if (target.Health + target.PhysicalShield <= _Player.GetSpellDamage(target, SpellSlot.Q))
                    {
                        var Qpred = Q.GetPrediction(target);
                ...
                {
                    if (target.Health + target.PhysicalShield <= _Player.GetSpellDamage(target, SpellSlot.W))
                    {
                        var Wpred = W.GetPrediction(target);
        ...
        {
            //var castPos = _Player.Position.Distance(Game.CursorPosRaw) <= Q.Range ? Game.CursorPosRaw : _Player.Position.Extend(Game.CursorPosRaw, Q.Range).ToVector2();
            if (Misc["Stack"].GetValue<MenuBool>().Enabled && Q.IsReady() &&
            (_Player.InShop()) && (Tear.IsOwned() || Manamune.IsOwned()))
            {
                Q.Cast();
            }
            ...
            (!_Player.InShop() && _Player.Position.CountEnemyHeroesInRange(2500) < 1 && !_Player.IsRecalling() && _Player.ManaPercent >= mana && !GameObjects.AllyMinions.Any(x => x.IsValidTarget(2000))
